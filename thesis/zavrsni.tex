...
Najosnovnij takav algoritam pretrage je pretraživanje s jednoliko cijenom (engl. \textit{uniform-cost search}). 
Algoritam koristi prioritetni red u kojeg stavlja čvorove s njihovom ukupnom udaljenosti od početnog stanja \(g(n)\), što osigurava da će svako stanje biti posjećeno sa najkraćim mogućim putem.
...
Heuristika se u tom kontekstu definira kao procijena udaljenosti do cilja. 
Npr. u situacijama kada je stanje definirano sa pozicijom, poput u primjeru na slici \ref{fig:basicGrid}, heuristička funkcija \(h(n)\) može biti zračna udaljenost pozicije od cilja.
