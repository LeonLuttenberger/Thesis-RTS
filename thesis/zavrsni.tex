...
\begin{minipage}{\textwidth}
    ...
    label={fig:codeStickman}]{codesnippets/stickman.txt}
\end{minipage}\
...
\par Uz pretpostavku da tekstura za ovu jedinicu uspješno napravljena, navedeni isječak koda je dovoljan da ona bude funkcionalna unutar igre, kao što je prikazano na slici~\ref{fig:stickman}.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{images/stickmanUnit.png}
	\caption{Nova jedinica se bori protiv postojećih}
	\label{fig:stickman}
\end{figure}

\par S obzirom na činjenicu da je jedinica samo dodana u igru, ostatak igre nezna za nju te se ona ne može prirodno pojaviti u igri.
Na slici~\ref{fig:stickman} nova jedinica je umjetno ubačena u igru pomoću šifri za varanje (engl. \textit{cheat codes}).
