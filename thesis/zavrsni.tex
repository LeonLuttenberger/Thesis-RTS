...
U popoglavlju \ref{sec:adaptation} prikazane su metode kojima se može postići prilagodba algoritma na promjenu.

\par Konačno, u potpoglavlju \ref{sec:grouping} opisan je algoritam \textit{Boids} pomoću kojeg se može ostaviti zajedničko kretanje određene grupe jedinica.
Ova optimizacija omogućava pojavu većeg broja jedinica u igrinom svijetu, jer se pretrage neće više izvršavati za sve jedinice u pokretu nego za svaku grupu.
...
\par Problem pretrage prostora možemo formulirati sa šestorkom
...
Ako definirano da se pločica u donjem lijevog predstavlja s koordinatama \((1, 1)\), tada standardna formulacija problema glasi:
\begin{itemize}
...
\par Unatoč tome što će algoritam uvijek pronaći najkraći put, ponekad će za to morati proširiti veliki broj stanja jer se u obzir uzima samo cijena puta, bez znanja o tome dali je taj čvor bliži cilju ili ne. 
...
Sva stanja u tom lokalnom prostoru također moraju biti u blizini agenta, odnosno samo određeni broj poteza udaljena.
...
Nakon prve pretrage, agent se pomaknuo do pozicije \((2, 3)\) kada se na  terenu uočena iznenadna promjena: na poziciju \((3, 3)\) dodana je prepreka.
...
U drugoj pretrazi, te se vrijednosti gledaju kao heurističke vrijednosti za njihova stanja, što sprječava pretragu da se usmjeri prema prijašnjoj početnoj poziciji, odnosno \((3, 1)\).
